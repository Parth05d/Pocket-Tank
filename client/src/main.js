...
socket.on('player-joined', (data) => {
  ...
  if (data.players.length >= 1) { // Host can start even with 1 for testing
...
// UI Handlers
btnCreate.onclick = () => {
  socket.emit('create-room', { nickname: inputNickname.value || 'Player 1' }, (res) => {
    if (res.success) {
...
btnComputer.onclick = () => {
  socket.emit('create-room', { nickname: inputNickname.value || 'Player', vsComputer: true }, (res) => {
    if (res.success) {
      currentRoom = res.roomId;
      displayRoomCode.textContent = currentRoom;
      switchScreen(screenWaiting);
...
btnJoin.onclick = () => {
  ...
  if(!roomId) return;
  socket.emit('join-room', { roomId, nickname: inputNickname.value || 'Player' }, (res) => {
    if (res.success) {
      currentRoom = roomId;
      displayRoomCode.textContent = currentRoom;
      switchScreen(screenWaiting);
